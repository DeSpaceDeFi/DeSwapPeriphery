...
library UniswapV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = DeSwapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        if (aToB) {
            uint256 amountOut = DeSwapLibrary.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = DeSwapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        IUniswapV2Pair pair = IUniswapV2Pair(
            DeSwapLibrary.pairFor(factory, tokenA, tokenB)
        );
